...
import { addCollaborationAPI, deleteCollaborationAPI, getPresentationCollaborationsAPI } from 'api/PresentationAPI';
...
const CollaborationList = () => {
	...
	const list = useQuery({
		...
		queryFn: async () => await getPresentationCollaborationsAPI(accessToken, code),
		enabled: false,
	});
	...
	const addCollaborationsFormik = useFormik({
		...
		onSubmit: async (values, { resetForm }) => {
			const response = await addCollaborationAPI(accessToken, code, values.email);
			const { meta } = response;
			if (meta.code === 200) {
	...
	return (
		<>
			...
			<Modal
				...
				footer={null}
			>
				<p className="invite-description">Collaborator là người có thể xem và chỉnh sửa presentation</p>
				<form
				...
				>
					...
					/>
					<div className="error">
						{addCollaborationsFormik.errors.email && <p>{addCollaborationsFormik.errors.email}</p>}
					</div>
