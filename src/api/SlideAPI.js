...
export const updateSlideAPI = async (accessToken, slideInfo) => {
	const response = await axios
		.put(`${process.env.REACT_APP_API_URL}/slides`, slideInfo, {
			headers: {
				'Content-Type': 'application/json',
				Authorization: `Bearer ${accessToken}`,
			},
...
export const deleteSlideAPI = async (accessToken, presentationCode, itemNo) => {
	const response = await axios
		.delete(`${process.env.REACT_APP_API_URL}/slides?${presentationCode}&${itemNo}`, {
			headers: {
				'Content-Type': 'application/json',
				Authorization: `Bearer ${accessToken}`,
			},
